...
use App\Services\Reporting\Jobs\GenerateReportExportJob;
...
class ReportService
{
    ...
    public function generate(string $type, array $data, string $format, ?int $templateId = null): \App\Models\GeneratedReport
    {
        ...
        // 2. Create DTO
        $dto = ReportFilterDTO::fromRequest($data, $type, $format, $templateId);

        // 3. Dispatch Job
        GenerateReportExportJob::dispatch($reportRecord->id, $dto);

        return $reportRecord;
    }

    public function saveTemplate(array $data):ReportTemplate
    {
